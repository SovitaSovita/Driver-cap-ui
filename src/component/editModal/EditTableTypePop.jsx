...
import { tourTypeSchema } from '../../utils/Validation';

import SaveAsOutlinedIcon from '@mui/icons-material/SaveAsOutlined';
import LockOutlinedIcon from "@mui/icons-material/LockOutlined";
import { useDispatch } from 'react-redux';
import { setIsGet } from '../../redux/slice/ListSlice';
import { update_tourType } from '../../redux/service/tourTypeService';

function EditTableTypePop(props) {
    ...
    const onSubmitHandle = (values) => {
        setIsLoading(true)
        update_tourType(values, fileImg.imageFile, oldData.id).then((res) => {
            if (res?.status == 200) {
        ...
        <div>
            ...
            <Transition appear show={isOpen} as={Fragment}>
                <Dialog as="div" open={!isOpen} className="relative z-10" static onClose={closeModal}>
                    <Transition.Child
                        as={Fragment}
                        enter="ease-out description-300"
                        enterFrom="opacity-0"
                        enterTo="opacity-100"
                        leave="ease-in description-200"
...
export default EditTableTypePop